      ...
      const value =
        account.type === 'crypto'
          ...
          : account.type === 'stock' || account.type === 'bond' || account.type === 'etf'
            ? convertCurrency(account.investmentHoldings?.reduce((sum, holding) => sum + holding.quantity * (holding.priceUsd ?? 0), 0) ?? account.balance, account.currency, 'VND', state.settings.usdToVndRate)
